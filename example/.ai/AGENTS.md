...
- **The camera is faked on the iOS Simulator**, which has no capture hardware.
  ...
  tracking instead of a fixed scene. Recognition is always real, and the live scanner asks for
  `RecognitionLevel.accurate` because Vision's `fast` path reports a flat 0.5 confidence. It is a no-op on a device and on Android, so those still
...
- Navigation: a landing hub (`HomeView`) pushes each feature screen with
  `Navigator.push(MaterialPageRoute)`, with no routing package and no tabs. A full-screen
...
- Gates: the example is held to the same bar as the package: `flutter analyze` **and**
  `dcm analyze lib` (DCM runs separately, and `flutter analyze` alone misses its rules),
  `dart format`, the host-side `flutter test`, and the on-device `flutter test integration_test`.
  Run all of them after changes here.
