...
- **Within a feature, `core` itself included, imports are relative**: a view importing
  its own `<feature>_view_model.dart`, a feature importing its own `data/…`, or a `core`
  file importing a sibling `core` file directly
  (`import '../widgets/platform/platform_card.dart';`, *not* the barrel, since a module needn't
...
- **VM-internal state stays plain.** Fields no widget observes (e.g. a
  `StreamSubscription`, a cached `Stopwatch`) are plain Dart fields, with no notifier
  ceremony.
...
- ★ Default to `should` for user preferences and UI toggle state, since it is declarative,
...
field, since the event and the state describe different things.
...
Avoid VM-leaking names like `setX`, `runX`, `commitX`, `forceX`, since those describe what
...
4. **`init()`**: lifecycle entry, setting up streams / subscriptions / the session.
...
- **Widget-state holding domain input belongs on the VM.** The package's
  ...
  construction and disposal, and the view binds directly
...
- **Widget-state describing pure UI presentation belongs on the view.** "This button is
  mid-async, show a spinner" is purely visual, with no VM logic and no other widget consumes
...
- **`PlatformCard` content `Padding`: `.all(16)`** by default, matching Material 3's
...
- **Plug missing widgets under [`platform/`](lib/features/core/widgets/platform/), one
  ...
  escape hatch. These are temporary, and they move into the base library later.
