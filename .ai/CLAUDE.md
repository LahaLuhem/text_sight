...
- **Lint with `flutter analyze`** — the project promotes many lints to `error:` in
  `analysis_options.yaml`; those are the contract, not suggestions. The Swift/Kotlin sides
  have no analyzer gate — apply [`../CODESTYLE.md`](../CODESTYLE.md)'s native conventions by
  hand.
