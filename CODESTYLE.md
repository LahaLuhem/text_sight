...
- **`final` by default for fields and locals.** Parameters are *not* required to be
  `final`, since `avoid_final_parameters` allows mutation-shaped parameters, and
...
- **Prefer abbreviations over initialisms for domain terms.** In code, comments,
  ...
  expand, since shorthand that's "obvious" to the author is opaque to the next reader and
  ...
  (`switch (x) { final cb => … }` is **out**, so spell it). The only carve-outs are the
...
- **Local-variable names carry a concise type-suffix.** Dart is strongly typed, but a
  ...
  stay single-word (`capture`, `line`, `level`), since the enclosing call site already pins
...
- **Unused closure parameters take the discard `_`, not a real name.** Don't declare an
  identifier you don't reference, since `_` makes the unused-ness immediate.
  ...
  Applies in dartdoc examples too, and `(_)` is the idiomatic Dart form and users
...
- **Don't rename callback params to disambiguate from a same-named outer-scope
  variable.** Dart's lexical scoping always picks the innermost binding, so there's no
...
- **Files mirror their primary public symbol.** `TextSightController` lives in
  `text_sight_controller.dart`, `TextSightView` in `text_sight_view.dart`, and each result
  ...
  `file_names` enforces snake_case, and the file ↔ symbol pairing is by convention.
...
- **Blank lines separate logical chunks within a method.** Group guard checks, setup,
  ...
  the native sides too (see [Swift](#swift-ios) / [Kotlin](#kotlin-android)).
...
- **No magic numbers in `lib/` code.** Pull constants to named `static const`s with a
  ...
  origin is obvious.
- **Inline single-use defaults, and don't promote to a named `kDefault…` constant.** A
  ...
  substitution. When the value appears only as one constructor's parameter default, with no
  ...
  reader, since once inlined the dartdoc just spells out the literal: ``Defaults to `false` ``.

> Native-side magic numbers (recognition thresholds, pixel formats, EXIF orientation
> codes) follow the same rule, so name them. See [Swift](#swift-ios) /
...
- **Default to `final class`, and leave a class open only where extension is a designed-in seam.**
  The data models (`TextSightCapture`, `RecognizedLine`, `RecognizedElement`), the config
  (`TextSightOptions`), the controller, and the view are all `final`, since nothing is meant to
  ...
  first publish, since relaxing `final` later is non-breaking, tightening *to* it is not, so the
  ...
  classes are exempt, being already sealed or never hand-edited.
...
- **Value types override `toString`.** Immutable data classes (`TextSightCapture`,
  ...
  a meaningful string representation, as an expression-bodied one-liner placed after the
  constructors, before any static helpers. Opaque fields (controllers, listenables,
  builder callbacks) are omitted, since they add noise without informing the reader, and bare
  interpolation of a callable trips DCM's `avoid-missed-calls`. Pigeon-generated
  message classes are exempt, so don't hand-edit generated code to add `toString`.
...
Skip when it hurts readability, as `.new(…)` for unnamed constructors typically does, as
...
entirely, and remove it from any `show` clauses too (`unused_shown_name` flags orphans).
...
- **Empty literals without a slot.** `final lines = <RecognizedLine>[];`, where the local has
...
**Why.** The `spacing` form keeps `children` purely about content, and the layout metadata
...
reads as data, and a `.map(…).toList()` reads as a pipeline that incidentally produces data.
...
That rule is about *constructing* a literal, where `[for (…) …]` reads as data. This
...
`Future.syncValue(x)` is the SDK-recommended way to wrap a **known, non-future value**, and it sets
...
microtask, so it is *not* a synchronously-completing `SynchronousFuture`). Reach for `Future.value`
...
When you genuinely need to hand out a defensive copy of mutable internal state, say a getter
...
Public symbols carry `///` dartdoc that explains *why*, not *what*, since types already carry
...
**not** add a regular `import`, since that pulls the dependency into the runtime import graph
...
file's `library;` directive, and code imports stay as regular `import` lines. The `library;`
...
- **`no-empty-block`**: every block (function literal, `if`, `for`, `try`…) must
  ...
  catch clauses are excused. An empty callback is a violation, so give it work or a TODO.
...
- **Tests are BDD/Cucumber-style via [`bdd_framework`](https://pub.dev/packages/bdd_framework).**
  ...
  test around an explicit *system-under-test* and reads as living documentation, and the console
  output is Gherkin.
- **Parameterise inputs with examples and tables, never scattering literals through the body.**
  ...
  table, and the body only references them, so the same body covers every row.
...
analyzer gate** in CI, so these conventions are applied by hand, like the DCM rules above.
...
- **`guard … else { return }` for early exits, and never force-unwrap (`!`).** Optionals
...
- **Run Vision off the main thread, marshalling results back to main before the
...
- **`try?` to drop a failed *frame*, `do/catch` when a failure must surface.** A single
  ...
  error, so `do/catch` and propagate it to Dart.
- **Name things in full**, same as Dart: `pixelBuffer` not `pb`, `recognizedLine` not
  ...
  stay, and your own identifiers expand. Normalise Vision's bottom-left bounding boxes to
...
- **ML Kit and CameraX dependencies live only in `android/build.gradle.kts`.** They never
  appear as Dart `pubspec.yaml` dependencies, and that's the no-bundling contract from the
  Android side ([`APPENDIX.md#no-bundling`](./APPENDIX.md#no-bundling)).
- **Null-safety: prefer non-null types, and use `?.` / `?:` / `let`, never `!!`.** The
...
- **`imageProxy.close()` in `addOnCompleteListener` is mandatory.** With
  ...
  closed, since forgetting `close()` silently stalls the stream. Treat it as a correctness
  invariant, not a style nicety (it's also called out in
  [*Hard rules*](./.ai/AGENTS.md#hard-rules)).
- **Run ML Kit off the platform main thread, marshalling results back to main for the
...
- **Expression bodies for one-liners** (`fun foo() = …`), expand when a body needs
  ...
  to pixel coords in the rotated image space, so convert to top-left `[0,1]` **here**, with
...
- **Say it once, then stop.** Docs carry the *why* and the surprising bits, with no restating of a
...
- **APPENDIX.md is the source of truth for rationale.** Hard rules, pitfalls, and
  workflow stay in `.ai/AGENTS.md` and `.ai/CLAUDE.md`, and the "why we do it this way"
